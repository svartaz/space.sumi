...
export const yearIsLeap = (year: number): boolean =>
  (0 === year % 4 && 0 !== year % 100) || 0 === year % 400;

export const yearToDays = (year: number): number =>
  yearIsLeap(year) ? 366 : 365;

export const dateIsDst = (date: Date): boolean => {
...
export const dateToObject = (
  ...
  const day =
    (date.getTime() - new Date(year, 0, 1).getTime()) / 1000 / 60 / 60 / 24;
  ...
  return {
    ...
    day,
    unix:
      (date.getTime() - new Date("1970-01-01T00:00Z").getTime()) /
      (24 * 60 * 60 * 1000),
    text: `${year}d${Math.floor(day).toString().padStart(3, "0")}`,
  };
};

export const yearDayToDate = (year: number, day: number): Date =>
